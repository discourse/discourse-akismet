...
export default Ember.Controller.extend({
  ...
  actions: {
    refresh() {
      ...
      AkismetQueue.findAll().then(result => {
    ...
    confirmSpamPost(post) {
      this.set('performingAction', true);
      AkismetQueue.confirmSpam(post).then(() => {
    ...
    deleteUser(post) {
      bootbox.confirm(I18n.t('akismet.delete_prompt', {username: post.get('username')}), result => {
        if (result === true) {
          this.set('performingAction', true);
          AkismetQueue.deleteUser(post).then(() => {
            this.get('model').removeObject(post);
            this.incrementProperty('stats.confirmed_spam');
            this.decrementProperty('stats.needs_review');
          }).catch(genericError).finally(() => {
